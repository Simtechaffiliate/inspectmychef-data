...
CI-safe: the postback poll loops wrap page.evaluate() in try/except so they
survive "Execution context was destroyed" errors raised when the ASP.NET
postback navigation lands mid-poll. Locally the timing rarely triggered
this; in GitHub Actions the slower VM hit it every run.

Run:

    python scraper.py

Env vars: HEADLESS, MAX_PAGES, SKIP_GEOCODE, PAGE_SIZE.
"""
